...
// export const revalidate = 0; // 제거: 클라이언트 컴포넌트에서는 사용 불가
...
export default function Home() {
  ...
  const autoRefreshEnabled = useRef<boolean>(true); // 자동 갱신 상태를 ref로 관리
    ...
    try {
      const response = await fetch('/api/track', {
        ...
        headers: {
          ...
          'Cache-Control': 'no-cache, no-store', // 캐시 방지
        },
  ...
  // 자동 갱신 기능 관리
  useEffect(() => {
    // 컴포넌트 마운트 시 자동 갱신 활성화 상태 저장
    autoRefreshEnabled.current = isAutoRefresh;
    ...
    if (isAutoRefresh && trackingResult && !trackingResult?.status?.id?.includes('delivered')) {
      ...
      autoRefreshTimerRef.current = setInterval(() => {
        // 배송이 완료되지 않은 경우에만 갱신
        if (trackingNumber && carrier && !isLoading && autoRefreshEnabled.current) {
  ...
  useEffect(() => {
    const handleVisibilityChange = () => {
      if (document.visibilityState === 'visible') {
        // 페이지가 다시 보이면 자동 갱신 활성화 (이전에 활성화되어 있었다면)
        if (autoRefreshEnabled.current && trackingNumber) {
          handleTrackWithoutEvent();
        }
      }
    };
  ...
  }, [handleTrackWithoutEvent, trackingNumber]);
